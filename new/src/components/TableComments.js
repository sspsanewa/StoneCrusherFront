...
const headCells = [
    ...
    { id: 'name', numeric: false, label: 'Comment By', minWidth: 100 },
    { id: 'comment', numeric: false, label: 'Comment', minWidth: 100 },
    { id: 'comment_type', numeric: false, label: 'Comment Type', minWidth: 100 },
    {
        id: 'createDate', numeric: false, label: 'Comment On', minWidth: 100
    },
];
...
function EnhancedTableToolbar(props) {
    ...
    return (
        <Typography marginBottom={4} fontSize={18} color={'gray'}>Comments</Typography>
    );
}
...
export default function TableComments(props) {
    ...
    const [filteredRows, setFilteredRows] = React.useState(comments);
    ...
    const handleSearch = (searchTerm) => {
        setSearch(true)
        const filtered = comments.filter((row) =>
    ...
    const visibleRows = React.useMemo(
        () =>
            stableSort(setSearch, comments, getComparator(order, orderBy)).slice(
                page * rowsPerPage,
                page * rowsPerPage + rowsPerPage,
            ),
        [order, comments, orderBy, page, rowsPerPage],
    );
    ...
    React.useEffect(() => {
        ...
        const params = { action: 'get_musa_comment', musa_id: props.id };
        axios.get(`${Url}/manage_controller/musa_comments`, { params })
            .then(obj => {
                Console("res", obj.data.data.musa_arr)
                setComments(obj.data.data.musa_arr);
            })
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell align="left">
                                            {row.comment_type === 1 ? <Typography variant='outlined' size='small' style={{ height: '25px', color: '#00c853' }} >Text</Typography> :
